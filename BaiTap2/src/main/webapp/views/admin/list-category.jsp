                ...
                <table class="modern-table" id="categoryTable">
                    <thead>
                        <tr>
                            ...
                            <th style="width: 21%;">Hành động</th>
                        </tr>
                    </thead>
                    <tbody id="tableBody">
                        <c:if test="${empty cateList}">
                            <tr><td colspan="5" class="empty-state">Chưa có danh mục nào. Hãy thêm danh mục mới!</td></tr>
                        </c:if>
                        <c:forEach items="${cateList}" var="cate" varStatus="STT">
                            <tr>
                                <td style="font-weight: 500; color: #495057;">${STT.index + 1}</td>
                                <c:url value="/image?fname=${cate.icon}" var="imgUrl"/>
                                <td><img class="avatar-box" src="${imgUrl}" alt="${cate.name}"/></td>
                                <td style="font-weight: 500; color: #212529;">${cate.name}</td>
                                <td class="price-col"><fmt:formatNumber value="${cate.price}" type="number" groupingUsed="true"/> đ</td>
                                <td>
