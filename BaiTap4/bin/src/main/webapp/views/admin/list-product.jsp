...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .search-box { display: flex; align-items: center; gap: 6px; font-size: 13px; color: #6c757d; }
    .search-box input {
    ...
    .modern-table th {
        background: #f8f9fa; color: #495057; font-weight: 600; font-size: 13px;
        padding: 14px 20px; border-bottom: 2px solid #dee2e6; text-align: left;
    }
    .modern-table td {
        padding: 14px 20px; border-bottom: 1px solid #e9ecef;
    ...
    .avatar-box {
        width: 100px; height: 75px; object-fit: cover; border-radius: 10px;
        border: 1px solid #dee2e6; box-shadow: 0 2px 8px rgba(0,0,0,0.06);
    }
    .action-link {
        text-decoration: none; font-weight: 500; font-size: 13px; color: #1c7ed6;
        padding: 6px 14px; border-radius: 20px;
        background: #e7f5ff; border: 1px solid #a5d8ff;
        transition: all 0.2s; display: inline-block; margin-right: 6px;
    }
    ...
    .empty-state {
        text-align: center; color: #868e96; padding: 50px 20px;
        font-size: 14px; font-style: italic;
    }
    .table-footer {
        padding: 12px 20px; font-size: 13px; color: #868e96;
        border-top: 1px solid #dee2e6; background: #f8f9fa;
    }
    .alert-success {
        background: #ebfbee; color: #2b8a3e; padding: 14px 20px;
        border-radius: 10px; margin-bottom: 20px; font-size: 14px; font-weight: 500;
    ...
    .cate-badge {
        background: #e7f5ff; color: #1c7ed6; padding: 3px 10px;
...
<body>
    ...
    <div class="workspace-wrapper">
        ...
        <div class="content-canvas">
            ...
            <p class="page-subtitle">Nơi bạn có thể quản lý sản phẩm của mình</p>
            ...
            <c:if test="${param.message == 'add_success'}">
                <div class="alert-success" id="alertMsg">
                    <span><i class="fas fa-check-circle" style="margin-right: 8px;"></i>Thêm sản phẩm mới thành công!</span>
                    <button class="close-btn" onclick="this.parentElement.style.display='none'">&times;</button>
                </div>
            </c:if>
            <c:if test="${param.message == 'edit_success'}">
                <div class="alert-success" id="alertMsg">
                    <span><i class="fas fa-check-circle" style="margin-right: 8px;"></i>Cập nhật sản phẩm thành công!</span>
                    <button class="close-btn" onclick="this.parentElement.style.display='none'">&times;</button>
                </div>
            </c:if>
            ...
            <div class="data-card">
                ...
                <table class="modern-table" id="productTable">
                    ...
                    <tbody id="tableBody">
                        <c:if test="${empty productList}">
                            <tr><td colspan="7" class="empty-state">Chưa có sản phẩm nào. Hãy thêm sản phẩm mới!</td></tr>
                        </c:if>
                                ...
                                <td>
                                    <c:choose>
                                        <c:when test="${p.image != null && p.image.startsWith('http')}">
                                            <img class="avatar-box" src="${p.image}" alt="${p.productName}"/>
                                        </c:when>
                                        <c:otherwise>
                                            <c:url value="/image?fname=${p.image}" var="imgUrl"/>
                                            <img class="avatar-box" src="${imgUrl}" alt="${p.productName}"/>
                                        </c:otherwise>
                                    </c:choose>
                                </td>
                                <td style="font-weight: 500; color: #212529;">${p.productName}</td>
                                <td>
                                    <c:if test="${p.category != null}">
                                        <span class="cate-badge">${p.category.name}</span>
                                    </c:if>
                                </td>
                                <td class="price-col"><fmt:formatNumber value="${p.price}" type="number" groupingUsed="true"/> đ</td>
                                <td style="color: #6c757d; font-size: 13px;">${p.createdDate}</td>
                                <td>
                                    <a class="action-link" href="<c:url value='/admin/product/edit?id=${p.productId}'/>">Sửa</a>
                                    <a class="action-link link-delete" href="<c:url value='/admin/product/delete?id=${p.productId}'/>" onclick="return confirm('Bạn có chắc chắn muốn xóa sản phẩm này?')">Xóa</a>
                                </td>
                            </tr>
                        </c:forEach>
                    </tbody>
                ...
                <div class="table-footer">
                    <c:choose>
                        <c:when test="${empty productList}">Hiển thị 0 sản phẩm</c:when>
                        <c:otherwise>Hiển thị ${productList.size()} sản phẩm</c:otherwise>
                    </c:choose>
                </div>
            </div>
        </div>
